...
export default function AdminPaymentsPage() {
  ...
  const statusCfg: Record<string, { label: string; icon: React.ElementType; badge: string }> = {
    completed: { label: tCommon('statusSucceeded'), icon: CheckCircle2, badge: 'text-[var(--color-success)] bg-[var(--color-success)]/10 border-[var(--color-success)]/20' },
    failed: { label: tCommon('statusFailed'), icon: XCircle, badge: 'text-[var(--color-error)] bg-[var(--color-error)]/10 border-[var(--color-error)]/20' },
    refunded: { label: tCommon('statusRefunded'), icon: RefreshCw, badge: 'text-[var(--color-warning)] bg-[var(--color-warning)]/10 border-[var(--color-warning)]/20' },
  };
      ...
      <header className="px-8 py-6 border-b border-white/5 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--color-text-secondary)] mt-0.5">
            {loading ? t('loading') : t('totalCount', { count: payments.length })}
          </p>
        </div>
      </header>
      ...
      <div className="px-8 py-6 max-w-7xl mx-auto space-y-5">
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-[var(--color-card)] rounded-xl border border-white/5 p-4 text-center">
            <p className="text-2xl font-bold tabular-nums text-[var(--color-success)]">{totalRevenue.toLocaleString(locale)} TL</p>
            <p className="text-xs text-[var(--color-text-secondary)] mt-1">{t('metricSuccessful', { count: completed.length })}</p>
          </div>
          <div className="bg-[var(--color-card)] rounded-xl border border-white/5 p-4 text-center">
            <p className="text-2xl font-bold tabular-nums text-[var(--color-error)]">{failCount}</p>
            <p className="text-xs text-[var(--color-text-secondary)] mt-1">{t('metricFailed')}</p>
          </div>
          <div className="bg-[var(--color-card)] rounded-xl border border-white/5 p-4 text-center">
            <p className="text-2xl font-bold tabular-nums text-[var(--color-warning)]">{refundCount}</p>
            <p className="text-xs text-[var(--color-text-secondary)] mt-1">{t('metricRefunded')}</p>
          </div>
        </div>
        ...
        <div className="bg-[var(--color-card)] rounded-2xl border border-white/5 overflow-hidden">
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-white/5 bg-[var(--color-sidebar)]/50">
                {[t('tableUser'), t('tablePackage'), t('tableAmount'), t('tableCredits'), t('tableDate'), t('tableStatus')].map((h) => (
            ...
            <tbody className="divide-y divide-white/5">
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-5 py-16 text-center text-sm text-[var(--color-text-secondary)]">{t('loading')}</td>
                </tr>
              ) : filtered.map((pay) => {
                ...
                return (
                  <tr key={pay.id} className="hover:bg-white/[0.025] transition-colors">
                    ...
                    <td className="px-5 py-4 text-sm tabular-nums font-semibold text-[var(--color-text-primary)]">{pay.amount.toLocaleString(locale)} TL</td>
                    <td className="px-5 py-4 text-sm tabular-nums text-[var(--color-text-primary)]">{pay.credits.toLocaleString(locale)}</td>
                    <td className="px-5 py-4 text-xs text-[var(--color-text-secondary)]">
                      {pay.createdAt ? new Date(pay.createdAt).toLocaleDateString(locale) : '—'}
                    </td>
          ...
          {!loading && filtered.length === 0 && (
            <div className="py-16 text-center text-sm text-[var(--color-text-secondary)]">{t('noResults')}</div>
          )}
        </div>
      </div>
    </div>
